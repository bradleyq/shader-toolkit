...
void main() {
    ...
    if (index != -1) {
        if (isSky > 0.5) {
            ...
            // store vertical and horizontal FOV in control pixels
            else if (index >= 3 && index <= 4) {
                fragColor = vec4(encodeFloat(atan(ProjMat[index - 3][index - 3])), 1.0);
            }  
            // store far clipping plane in control pixels
            else if (index == 5) {
                fragColor = vec4(encodeFloat(ProjMat[3][2] / (ProjMat[2][2] + 1) / PROJFARSCALE), 1.0);
            }
